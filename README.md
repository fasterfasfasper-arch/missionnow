...
1. **ショートカット(一番ラク)**: KLMSの アカウント → 設定 → 「+新しいアクセストークン」でトークンを発行し、ショートカットアプリで「URLの内容を取得」(ヘッダ `Authorization: Bearer <トークン>`)→「クリップボードにコピー」→「Appを開く: Mission Now」。ホーム画面に置けば1タップです。アプリ側では「クリップボードから取り込む」を押します。
